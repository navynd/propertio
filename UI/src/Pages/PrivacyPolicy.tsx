...
const PrivacyPolicy: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const data = await legalService.getLegal({ page: "privacy", country: countryCode });
        if (cancelled) return;
  ...
  const pageTitle = settings.privacyPageTitle || "Privacy policy";
      ...
      <div className="terms-top-section">
        <Container>
          <div className="terms-top-ulsection">
            ...
            <p className="terms-top-ul-title">{pageTitle}</p>
          </div>
