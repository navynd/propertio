...
import { legalService, type LegalCountry, type LegalDocument, type LegalPageSettings } from "../services/legalService";

const Terms: React.FC = () => {
  const [tabValue, setTabValue] = useState(0);
  const [openDropdown, setOpenDropdown] = useState(false);
  const [loading, setLoading] = useState(true);
  const [settings, setSettings] = useState<LegalPageSettings>({});
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        const data = await legalService.getLegal({ page: "terms", country: countryCode });
        if (cancelled) return;
    ...
    <div className="terms-main-section">
      <div className="terms-top-section">
        <Container>
          <div className="terms-top-ulsection">
            ...
            <p className="terms-top-ul-title">Terms & conditions</p>
          </div>

          <div className="terms-title-row">
            <h2 className="terms-top-title">{settings.termsPageTitle || "Terms and conditions"}</h2>
      ...
      <Container>
        <Box mt={3}>
          ...
          {settings.contactBlock?.supportEmail ? (
            <div className="terms-content-section mt-4">
              <h3 className="terms-content-title">
                {settings.contactBlock.sectionTitle || "Contact us"}
              </h3>
              <p className="terms-content-text">
                Email:{" "}
                <a href={`mailto:${settings.contactBlock.supportEmail}`}>
                  {settings.contactBlock.supportEmail}
                </a>
              </p>
            </div>
