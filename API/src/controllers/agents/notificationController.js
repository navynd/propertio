...
const getAuthAgentId = (req = {}) => req?.user?.agentId || req?.user?.id || req?.user?._id;
 ...
 *                           title:
 *                             type: string
 *                             example: Property Allocation Updated
 *                           message:
 *                             type: string
 *                             example: Your allocation was updated.
 *                           notificationType:
 *                             type: string
 ...
 *                             type: string
 *                             nullable: true
 *                             example: "/agents/allocated-properties"
 *                           actionText:
 *                             type: string
 *                             nullable: true
 *                             example: View
...
const listNotifications = asyncHandler(async (req, res) => {
  const agentId = getAuthAgentId(req);
  if (!agentId) {
  ...
  try {
    const filter = {
      'recipient.recipientId': new Types.ObjectId(agentId),
    ...
    logger.error('Agent list notifications error', {
      error: err.message,
      agentId,
    });
...
/**
 * @swagger
 * /agents/notifications/{id}/read:
 *   put:
 *     summary: Mark an agent notification as read
 *     tags: [Agents]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
const markAsRead = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    logger.error('Agent mark notification as read error', {
      error: err.message,
      notificationId: id,
      agentId,
    });
...
/**
 * @swagger
 * /agents/notifications/read-all:
 *   put:
 *     summary: Mark all agent notifications as read
 *     tags: [Agents]
 *     security:
 *       - bearerAuth: []
 *     responses:
...
const markAllAsRead = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    logger.error('Agent mark all notifications as read error', {
      error: err.message,
      agentId,
    });
 ...
 * /agents/notifications:
 *   delete:
 *     summary: Delete agent notifications (single or multiple)
 *     tags: [Agents]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
...
const deleteNotifications = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    logger.error('Agent delete notifications error', {
      error: err.message,
      agentId,
      ids,
    });
