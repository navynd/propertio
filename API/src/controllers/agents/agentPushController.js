...
const getAuthAgentId = (req = {}) =>
  req?.user?.agentId || req?.user?.id || req?.user?._id;

/**
 * @swagger
 * /agents/notifications/push/register:
 *   post:
 *     summary: Register or refresh authenticated agent push token
 *     tags: [Agents]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
...
const registerPushToken = asyncHandler(async (req, res) => {
  ...
  try {
    await registerFcmToken('agent', agentId, req.body || {});
    ...
    logger.error('agent registerPushToken failed', { error: error.message, agentId });
...
/**
 * @swagger
 * /agents/notifications/push/unregister:
 *   delete:
 *     summary: Unregister authenticated agent push token
 *     tags: [Agents]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
const unregisterPushToken = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const result = await unregisterFcmToken('agent', agentId, merged);
    ...
    logger.error('agent unregisterPushToken failed', { error: error.message, agentId });
