...
const getAuthDeveloperId = (req = {}) => req?.user?.developerId || req?.user?.id || req?.user?._id;
 ...
 *   get:
 *     summary: Get notifications for authenticated developer
 *     tags: [Developers]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *                           title:
 *                             type: string
 *                             example: Project Deal Approved
 *                           message:
 *                             type: string
 *                             example: Deal has been approved for Marina Heights.
 *                           notificationType:
 *                             type: string
 *                             example: alert
 *                           priority:
...
const listNotifications = asyncHandler(async (req, res) => {
  ...
  try {
    const filter = {
      'recipient.recipientId': new Types.ObjectId(developerId),
    ...
    logger.error('Developer list notifications error', {
      error: err.message,
      developerId,
    });
...
/**
 * @swagger
 * /developers/notifications/{id}/read:
 *   put:
 *     summary: Mark a developer notification as read
 *     tags: [Developers]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
const markAsRead = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    logger.error('Developer mark notification as read error', {
      error: err.message,
      notificationId: id,
      developerId,
    });
...
/**
 * @swagger
 * /developers/notifications/read-all:
 *   put:
 *     summary: Mark all developer notifications as read
 *     tags: [Developers]
 *     security:
 *       - bearerAuth: []
 *     responses:
...
const markAllAsRead = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    logger.error('Developer mark all notifications as read error', {
      error: err.message,
      developerId,
    });
 ...
 * /developers/notifications:
 *   delete:
 *     summary: Delete developer notifications (single or multiple)
 *     tags: [Developers]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
...
const deleteNotifications = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    logger.error('Developer delete notifications error', {
      error: err.message,
      developerId,
      ids,
    });
