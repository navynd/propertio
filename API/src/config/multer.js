...
// Allow images, documents (PDF, DOC/DOCX), and videos for report attachments
const reportFileFilter = (req, file, cb) => {
  ...
  return cb(
    createInvalidTypeError(
      file,
      'Only image, document (PDF/DOC/DOCX), or video (MP4/WebM) files are allowed'
    ),
    false
  );
};
...
// Report attachments (images + documents + videos)
const reportAttachmentsUpload = (fieldName = 'attachments', maxCount = 10) =>
  multer({
    storage: memoryStorage,
    fileFilter: reportFileFilter,
    limits: {
      fileSize: Math.max(MAX_IMAGE_SIZE, MAX_DOCUMENT_SIZE, MAX_VIDEO_SIZE),
    },
