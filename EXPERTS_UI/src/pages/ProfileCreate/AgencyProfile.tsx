...
import { EyeIcon, DownArrowIcon, SearchIcon, UploadIcon, VerifiedIcon } from "../../components/CustomFile/icons";
...
type AgencyProfileMasterResponse = {
...
const buildFullPhoneForAgencyOtp = (dialCode: string, localDigits: string) => {
...
const AgencyProfile = () => {
    ...
    useEffect(() => {
        ...
        const loadMasterData = async () => {
            try {
                ...
                const res = await apiClient.get<AgencyProfileMasterResponse>(
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        apiClient
            .post<UploadProfilePictureResponse>("/auth/agency/upload-profile-picture", form, { auth: false })
            .then((res) => {
    ...
    const handleSendEmailOtp = async () => {
        ...
        try {
            await apiClient.post("/auth/agency/send-email-otp", { email: email.trim().toLowerCase() }, { auth: false });
    ...
    const handleSendPhoneOtp = async () => {
        ...
        const fullPhone = buildFullPhoneForAgencyOtp(selectedCountry.dialCode, localDigits);
        ...
        try {
            ...
            // TEMP: backend returns OTP until SMS gateway is integrated.
            if (response?.otp) {
    ...
    const handleDocChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const allowed = files.filter((f) => /\.(pdf|doc|docx)$/i.test(f.name));
        if (allowed.length !== files.length) {
            toast.error("Invalid file", "Only PDF, DOC, or DOCX files are allowed.");
        }
        ...
        try {
            ...
            const names = docs
                ...
                .filter((n) => /\.(pdf|doc|docx)$/i.test(n));
            if (names.length) {
    ...
    const handleCreateProfile = async () => {
        ...
        if (!agencyName.trim()) {
            toast.error("Missing name", "Agency name is required.");
            return;
        }
        const localDigits = phoneNumber.replace(/\D/g, "");
        if (!localDigits) {
        ...
        if (!orn.trim()) {
            toast.error("Missing ORN", "Office registration number is required.");
            return;
        }
        if (!address.trim()) {
        ...
        if (uploadedRegistrationFilenames.length === 0) {
            toast.error("Missing documents", "Upload at least one registration document (PDF, DOC, or DOCX).");
            return;
        }
        ...
        try {
            await apiClient.post(
                `/auth/agency/accept-invitation?token=${encodeURIComponent(invitationToken)}`,
                {
            ...
            toast.error("Submit failed", getApiErrorMessage(error, "Could not create agency profile."));
        } finally {
    ...
    return (
        <div className="min-h-screen bg-[#F5F5F5] flex flex-col items-center pt-[40px] pb-[30px] px-[20px] font-['Outfit',sans-serif]">
            <div className="mb-[40px]">
                <img src={logoimg} alt="Molumulk Providers" className="h-[40px]" />
            </div>
            ...
            <div className="relative w-full max-w-[1200px] bg-white rounded-[15px] p-[4px] flex flex-col md:flex-row">
                ...
                <div className="flex-1 flex flex-col relative w-full md:p-[24px] p-[16px]">
                                ...
                                <div className="relative" ref={nationalityDropdownRef}>
                                    ...
                                    >
                                        {selectedNationality ? (
                                            ...
                                            <span className="text-[14px] font-[Regular] text-[#AAAAAA]">Select</span>
                                        )}
                                        <DownArrowIcon
                        ...
                        <div className="flex flex-col gap-[20px]">
                            <div>
                                ...
                                >
                                    ...
                                    <p className="text-[#707070] text-[12px] font-[Regular] mt-[8px] mb-[24px]">PDF, DOC or DOCX — up to 5 files (10MB each suggested)</p>
                                    <button
                                        type="button"
                    ...
                    <div className="flex items-center justify-between lg:col-span-2 pt-[10px] mt-auto">
                        <button
                            type="button"
                            onClick={() => navigate("/")}
                ...
                <div className="w-full md:w-[350px] bg-[#F9F9F9] rounded-[20px] p-[40px_30px] flex flex-col items-center shrink-0">
                    <h3 className="text-[#222] font-[Bold] text-[18px] mb-[40px] w-full text-center">Agency profile picture</h3>

                    <div className="w-[180px] h-[180px] border-2 border-white bg-[rgba(34,34,34,0.10)] shadow-[2px_0_15px_0_rgba(0,0,0,0.15)] rounded-[15px] mb-[40px] flex items-center justify-center overflow-hidden shrink-0">
                        {selectedImage ? (
                            <img src={selectedImage} alt="Profile preview" className="w-full h-full object-cover" />
                        ) : (
                            <svg xmlns="http://www.w3.org/2000/svg" width="90" height="100" viewBox="0 0 80 90" fill="none">
                                <path d="M22 15L54 0V90H22V15Z" fill="#D6D6D6" />
                                <path d="M57 32L78 15V90H57V32Z" fill="#C5C5C5" />
                                <rect x="30" y="32" width="14" height="6" rx="3" fill="#E8E8E8" />
                                <rect x="30" y="46" width="14" height="6" rx="3" fill="#E8E8E8" />
                                <rect x="30" y="60" width="14" height="6" rx="3" fill="#E8E8E8" />
                            </svg>
                        )}
                    </div>
            ...
            <p className="mt-[30px] text-[#222] text-[14px] font-[Regular]">Copyright © 2025 molumulk.ae. All rights reserved.</p>
        </div>
    );
};
