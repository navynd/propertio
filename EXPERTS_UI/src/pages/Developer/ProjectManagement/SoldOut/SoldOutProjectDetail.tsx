...
const SoldOutProjectDetail = () => {
    ...
    const [detailUnitProperties, setDetailUnitProperties] = useState<Array<Record<string, unknown>>>([]);
    ...
    useEffect(() => {
        if (!projectId) {
            ...
            navigate("/developer/project-management?tab=soldout");
            return;
        }
        let isMounted = true;
        setIsLoading(true);
        developerService
            ...
            .then((response) => {
                ...
                setDetailUnitProperties(
                    Array.isArray(response.unitProperties)
                        ? (response.unitProperties as Array<Record<string, unknown>>)
                        : []
                );
            })
            .catch((error: unknown) => {
            ...
            {!project && !isLoading ? (
                ...
                <>
                    ...
                    <ProjectDescription
                        ...
                        governmentFees={
                            typeof project?.governmentFees === "number" ? String(project.governmentFees) : "-"
                        }
                        unitProperties={
                            detailUnitProperties.length > 0
                                ? detailUnitProperties
                    ...
                    <div className="flex items-center justify-between gap-[10px] mt-[30px]">
                        <div className="flex items-center justify-end gap-[10px]">
                            <button className="cursor-pointer h-[44px] rounded-[10px] px-[20px] border border-[#222]  text-[#222] text-[14px] font-[Bold] inline-flex items-center gap-[5px]">
                                Discard
                            </button>
                            <button
...
export default SoldOutProjectDetail;
