...
const ReviewAssignment = () => {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    developerService
      .getProjectUnits({ projectId, bulk: true, page: 1, limit: 100 })
      .then((res) => {
        if (!mounted) return;
        ...
        (res.layouts || []).forEach((layoutItem) => {
          ...
          grouped[gid].layout.push({
            ...
            unitLabels: (layoutItem.units || []).map((u) => ({
              ...
              isClosed: String((u as { status?: string }).status || "").toLowerCase() === "closed",
            })),
          });
        });
  ...
  const allUnits = useMemo(
    () => groups.flatMap((tower) => tower.layout.flatMap((layout) => layout.unitLabels.map((unit) => ({ ...unit })))),
    [groups]
  );
  const allUnassignedIds = useMemo(() => allUnits.filter((unit) => unit.status === "Unassigned" && !unit.isClosed).map((unit) => unit.id), [allUnits]);

  useEffect(() => {
    if (!hasHydratedSelection) return;
    if (selectedAgencyIds.length === 0) return;
    const allSelectedUnion = new Set<string>();
    Object.values(agencyUnitMap).forEach((ids) => ids.forEach((id) => allSelectedUnion.add(id)));
    sessionStorage.setItem(
      "developer.bulkAssignSelection",
      JSON.stringify({
        projectId,
  ...
  }, [agencyUnitMap, hasHydratedSelection, projectId, selectedAgencyIds]);

  const toggleUnit = (unitId: string, status: UnitStatus, isClosed?: boolean) => {
    if (status === "Assigned" || isClosed || !activeAgencyId) return;
  ...
  const toggleSelectAll = () => {
    ...
    setAgencyUnitMap((prev) => {
      const current = prev[activeAgencyId] || [];
      const next = current.length === allUnassignedIds.length ? [] : allUnassignedIds;
      return { ...prev, [activeAgencyId]: next };
    });
  };
  ...
  const toggleSelectAllInLayout = (layout: Layout) => {
    if (!activeAgencyId) return;
    const layoutIds = layout.unitLabels.filter((u) => u.status === "Unassigned" && !u.isClosed).map((u) => u.id);
    const areAllSelected = layoutIds.every((id) => selectedUnitSet.has(id));
    setAgencyUnitMap((prev) => {
      const current = prev[activeAgencyId] || [];
      const next = areAllSelected ? current.filter((id) => !layoutIds.includes(id)) : [...new Set([...current, ...layoutIds])];
      return { ...prev, [activeAgencyId]: next };
    });
  };
  ...
  return (
    <div className="px-4 pb-6 pt-4 sm:px-6 lg:px-8 flex flex-col gap-[20px]">
      <div>
        <DeveloperHeader title="Review assignment" showBack={true} onBackClick={() => navigate(-1)} />
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_330px] gap-[20px]">
        <div className="rounded-[15px] bg-white border border-[rgba(34,34,34,0.06)] p-[20px]">
          <div className="flex items-center justify-between gap-3 mb-[6px]">
            ...
            <button type="button" onClick={toggleSelectAll} className="cursor-pointer flex items-center justify-center h-[33px] px-[14px] rounded-full border border-[rgba(34,34,34,0.10)] text-[12px] font-[SemiBold] text-[#222]">
              {activeAgencyUnitIds.length === allUnassignedIds.length ? "Deselect all" : "Select all"}
            </button>
          </div>
          <p className="text-[12px] font-[SemiBold] text-[#707070] mb-[8px]"><span className="text-[#222]">{activeAgencyUnitIds.length}</span> units selected</p>
          {isLoading ? (
              ...
              {groups.map((tower) => (
                <div key={tower.id} className="mb-[14px] last:mb-0">
                  <div className="flex items-center justify-between gap-3 mb-[20px]">
                    ...
                    <button type="button" className="cursor-pointer h-[33px] px-[12px] rounded-full border border-[rgba(34,34,34,0.10)] text-[12px] font-[SemiBold] text-[#222] bg-white">Select all property</button>
                  </div>
                  {tower.layout.map((layout) => {
                    const selectedInLayout = layout.unitLabels.filter((unit) => selectedUnitSet.has(unit.id)).length;
                    const layoutUnassignedIds = layout.unitLabels.filter((unit) => unit.status === "Unassigned").map((unit) => unit.id);
                    const isAllLayoutUnassignedSelected = layoutUnassignedIds.every((id) => selectedUnitSet.has(id));
                    return (
                      <div key={layout.id} className="rounded-[12px] bg-white border border-[rgba(34,34,34,0.06)] md:p-[30px] p-[20px] mb-[25px] last:mb-0">
                        <div className="flex items-center justify-between gap-4 mb-[12px]">
                          ...
                          <div className="flex items-center gap-[8px]">
                            ...
                            <button type="button" onClick={() => toggleSelectAllInLayout(layout)} className="cursor-pointer h-[33px] px-[12px] rounded-full border border-[rgba(34,34,34,0.10)] text-[12px] font-[SemiBold] text-[#222] bg-white">{isAllLayoutUnassignedSelected ? "Deselect all" : "Select all"}</button>
                          </div>
                        </div>
                        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-[8px]">
                          {layout.unitLabels.map((unit) => {
                            ...
                            return (
                              <button key={unit.id} type="button" disabled={isAssigned || isClosed} onClick={() => toggleUnit(unit.id, unit.status, isClosed)} className={`rounded-[10px] p-[15px_20px] flex flex-col gap-[9px] items-center justify-between text-[12px] font-[Bold] border border-[rgba(34,34,34,0.10)] leading-[1.2] transition-colors ${isSelected ? "cursor-pointer bg-[#222] text-white border-[#222] border border-[rgba(34,34,34,0.10)]" : isClosed ? "cursor-not-allowed bg-[#FFECEC] text-[#EA3934] border-[#F7C5C5]" : isAssigned ? "cursor-not-allowed bg-[rgba(8,50,174,0.10)] text-[#222] border-none" : "cursor-pointer bg-white text-[#222] border-[rgba(34,34,34,0.10)]"}`}>
                                <span className={`h-[15px] w-[15px] rounded-full border flex items-center justify-center text-[9px] leading-none ${isSelected ? "bg-[#EA3934] border-[#EA3934] text-white" : isAssigned ? "bg-[#DCE3F5] border-[#C4CEE9] text-transparent" : "bg-white border-[rgba(34,34,34,0.12)] text-transparent"}`}>{isSelected && <TickIcon width={8} height={6} />}</span>
                                <span>{unit.name}</span>
                              </button>
                            );
                          })}
        ...
        <div className="rounded-[15px] bg-white md:p-[30px] p-[20px] h-fit">
          ...
          <div className="flex flex-col gap-[10px] mb-[24px]">
            {selectedAgencyItems.map((agency) => {
              ...
              return (
                <button key={agency.id} type="button" onClick={() => setActiveAgencyId(agency.id)} className={`h-[44px] w-full rounded-full border px-[14px] text-[14px] font-[Medium] flex items-center justify-between transition-colors ${isActive ? "border-[#222] bg-white text-[#222]" : "border-[rgba(34,34,34,0.10)] text-[#222]"}`}>
          ...
          <div className="border-t border-[rgba(34,34,34,0.06)] pt-[18px]">
            <div className="rounded-[12px] bg-[#F5F5F5] md:p-[20px] p-[15px] mb-[14px]">
              ...
              <div className="flex items-center justify-between text-[14px] mb-[6px]">
                ...
                <span className="text-[#222] font-[Bold]">{activeAgencyUnitIds.length}</span>
              </div>
