...
import type { PropertyDetailsFormValue } from "./components/PropertyDetailsStep";
import type {
    PropertyMediaUploadFormValue,
    PropertyMediaUploadPreviewValue,
} from "../../../Agent/PropertiesManagement/AddProperty/AddProjectComponents/PropertyMediaUpload";
import type { OtherDetailsFormValue } from "../../../Agent/PropertiesManagement/AddProperty/AddProjectComponents/OtherDetailsStep";
import type { PropertyLocationFormValue } from "../../../Agent/PropertiesManagement/AddProperty/AddProjectComponents/LocationAdd";

type EditTabId = "status" | "media" | "other" | "location";

type AgencyPropertyDetail = {
    _id?: string;
    id?: string;
    title?: string;
    description?: string;
    listingType?: { _id?: string; transaction?: string };
    propertyType?: { _id?: string };
    amenities?: Array<{ _id?: string }>;
...
const EditProperty = () => {
    ...
    const uploadAndCacheMedia = useCallback(async () => {
        ...
        const response = await agencyService.uploadPropertyMedia(formData);
    ...
    const loadProperty = useCallback(async () => {
        ...
        let vidBase = "";
        try {
            const urlRes = await agencyService.getSupportedUrlsMasterData();
            ...
            const root =
                anyRes?.supportedUrls?.propertyUrl || anyRes?.supportedurls?.propertyUrl || anyRes?.propertyUrl;
            imgBase = root?.img || "";
            vidBase = root?.vid || "";
        ...
        try {
            const property = (await agencyService.getPropertyById(propertyId)) as AgencyPropertyDetail;
            const propertyObjectId = String(property?._id || property?.id || "");
    ...
    const saveStatusTab = async () => {
        ...
        setSavingTab("status");
        try {
            await agencyService.updateProperty(propertyId, {
    ...
    const saveLocationTab = async () => {
        ...
        try {
            await agencyService.updateProperty(propertyId, {
                location: {
                    fullAddress:
                        locationDetails.formattedAddress || propertyDetails.propertyAddress.trim() || undefined,
                    city: locationDetails.city,
                    zone: locationDetails.zone,
    ...
    return (
        <div className="px-4 pb-6 pt-4 sm:px-6 lg:px-8 flex flex-col gap-[20px]">
            <AgencyHeader
                title="Edit property"
                showBack={true}
                onBackClick={() => navigate("/agency/listings")}
            />
                            ...
                            return (
                                <button
                                    ...
                                    className={`cursor-pointer p-[20px_30px] border-b-2 text-[13px] font-[SemiBold] whitespace-nowrap transition-colors ${
                                        isActive ? "text-[#0832AE] border-[#0832AE]" : "text-[#222] border-transparent"
                                    }`}
                                >
                                    {tab.label}
                                </button>
