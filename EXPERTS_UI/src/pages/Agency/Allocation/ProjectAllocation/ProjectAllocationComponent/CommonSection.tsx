...
const emptyStateClass =
  "flex min-h-[180px] items-center justify-center rounded-[12px] border border-dashed border-[rgba(34,34,34,0.12)] bg-[#FAFAFA] px-4 py-8 text-center text-[14px] font-[Regular] text-[#707070]";

/** Same fixed heights as masterplan / video media (`img` / `video` classes). */
...
const CommonSection = ({ project }: { project: any }) => {
  const [galleryPage, setGalleryPage] = useState(0);
  const [openFaqId, setOpenFaqId] = useState<number | null>(null);
  const galleryImagess = project?.images || [];
  ...
  const masterPlanImage = resolveMediaRef(
    Array.isArray(project?.masterPlan) ? project.masterPlan[0] : undefined,
  );
  const hasVideo = Boolean(
    project?.videoTour && String(project.videoTour).trim(),
  );
  const tourUrl =
    project?.virtualTour360 && String(project.virtualTour360).trim();
  ...
  useEffect(() => {
    ...
    setOpenFaqId(faqs.length ? 0 : null);
  }, [project?.projectId, (project?.faqs || []).length]);
    ...
    <>
      <div className="rounded-[15px] bg-white md:p-[30px] p-[20px] flex flex-col gap-8 sm:gap-10 min-w-0">
        <section>
          ...
          {agencies.length === 0 ? (
            <div className={emptyStateClass}>No authorized agencies.</div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              {agencies.map((agency: any) => (
                <div
                  key={agency._id || agency.agencyName}
                  className="flex items-center gap-3 rounded-[10px] bg-[#F5F5F5]  p-[5px_10px_5px_4px]"
                >
                  <img
                    src={toImageUrl(agency.profilePicture, "agency")}
                    alt=""
                    className="h-[80px] w-[80px] shrink-0 rounded-[10px] object-cover"
                  />
                  <span className="text-[15px] font-[Bold] text-[#222] leading-[165%] truncate">
                    {agency.agencyName}
                  </span>
                </div>
              ))}
      ...
      <div className="rounded-[15px] bg-white md:p-[30px] p-[20px] flex flex-col gap-8 sm:gap-10 min-w-0">
        <section>
          ...
          {amenities.length === 0 ? (
            <div className={emptyStateClass}>No amenities listed.</div>
          ) : (
      ...
      <div className="rounded-[15px] bg-white  min-w-0">
        <div className="flex items-center justify-between gap-4 md:p-[30px_30px_20px_30px] p-[20px]">
        ...
        {galleryImagess.length === 0 ? (
          <div className={emptyStateClass}>No images available.</div>
        ) : (
          <div className="grid grid-cols-2 min-[400px]:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-[3px] sm:gap-1">
            {visibleGallery.map((item: any, idx: number) => (
              <div
                key={item._id || item.url || `${galleryPage}-${idx}`}
          ...
          <section className="rounded-[15px] bg-white md:p-[30px] p-[20px] min-w-0 flex flex-col">
            ...
            {hasVideo ? (
              <div className="relative overflow-hidden rounded-[12px] bg-[#F0F0F0]">
                <video
                  src={toVideoUrl(project?.videoTour)}
                  controls
                  className="w-full h-[220px] sm:h-[260px] lg:h-[280px] object-cover"
      ...
      <div className="rounded-[15px] bg-white p-5 md:p-[30px] p-[20px] min-w-0">
        ...
        <div className="flex flex-col gap-4 text-[14px] font-[Regular] text-[#222] leading-[165%]">
          <RichTextContent
            content={project?.aboutProject}
            className="text-[14px] font-[Regular] text-[#222] leading-[165%]"
            emptyFallback="No project description available."
          />
        </div>
      </div>
      <div className="rounded-[15px] bg-white md:p-[30px] p-[20px] min-w-0">
        ...
        {faqs.length === 0 ? (
          <div className={emptyStateClass}>No FAQs available.</div>
        ) : (
              ...
              return (
                <div key={index} className="min-w-0">
                  <button
                    type="button"
                    onClick={() =>
                      setOpenFaqId((prev) => (prev === index ? null : index))
                    }
