...
const PasswordOtpModal: React.FC<PasswordOtpModalProps> = ({
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    setOtp(next);
    const nextFocusIndex = Math.min(digits.length, 6) - 1;
    if (nextFocusIndex >= 0) {
      ...
      <div className="relative bg-white w-full md:max-w-[560px] h-auto max-h-[90vh] rounded-t-2xl md:rounded-[24px] flex flex-col transform transition-all duration-300">
        ...
        <div className="md:p-[0px_50px_60px_50px] p-[0px_20px_30px_20px]">
          <div className="flex flex-col items-center text-center">
            <img src={logo} alt="Estatehub" className="w-[138px] mb-[31px]" />
            ...
            <div className="flex items-center justify-center gap-[14px] mb-[24px]">
              {otp.map((digit, index) => (
                <input
                  ...
                  onPaste={handlePaste}
                  className={`
                    w-[56px] h-[56px] rounded-[14px] text-center text-[20px] outline-none border
            ...
            <button
              ...
              className={`
                ...
                ${isOtpComplete && !isVerifying
                  ? "bg-[var(--primary-color,#1F3D51)] text-white cursor-pointer"
                  : "bg-[var(--primary-color,#1F3D51)] text-white opacity-[0.4] cursor-not-allowed"}
              `}
            >
            ...
            <div className="flex items-center justify-center gap-[8px] text-[28px]">
              <span className="font-[Regular] text-[#222] text-[14px]">
                Didn&apos;t receive the email?
              </span>
              <button
                type="button"
                className="cursor-pointer font-[Bold] text-[var(--primary-color,#1F3D51)] text-[14px]"
                onClick={onResendOtp}
