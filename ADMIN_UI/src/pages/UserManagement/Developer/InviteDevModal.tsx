  ...
  const handleInvite = async () => {
    ...
    if (!trimmedName || !trimmedEmail) {
      push({
        ...
        description: "Please enter developer name and email.",
      });
      return;
    }
    ...
    try {
      ...
      await developersService.inviteDeveloper({
        name: trimmedName,
        email: trimmedEmail,
      });
      push({
        ...
        description: "Developer invitation sent successfully.",
      });
      onClose();
      onInviteSuccess?.();
    } catch (error) {
      ...
      <div className="relative bg-white w-full md:max-w-[480px] h-auto transform transition-all duration-300 rounded-t-[15px] md:rounded-[15px] max-h-[90vh] flex flex-col overflow-hidden">
        <div className="shrink-0 flex justify-end p-[20px_20px_0px_20px]">
          <button
            ...
            className="cursor-pointer h-[40px] w-[40px] rounded-[12px] border border-[rgba(34,34,34,0.10)] bg-white flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <div className="shrink-0 p-[20px_20px_20px_20px] md:p-[30px_50px_60px_50px]">
          <button
            ...
            disabled={isSubmitting}
            className="h-[44px] w-full rounded-[10px] bg-[#6A3CA8] text-white text-[14px] font-[Bold] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          >
