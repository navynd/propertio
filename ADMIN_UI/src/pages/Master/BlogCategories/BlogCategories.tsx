...
import type { BlogCategoryListCounts, BlogCategoryRecord } from "../../../types/api";
import BlogCategoriesModal, { type BlogCategoryFormPayload } from "./BlogCategoriesModal";
import { formatJobTitleDate } from "../Jobtitle/jobTitleData";

const tableGrid =
  "grid-cols-[minmax(120px,1.2fr)_1fr_0.6fr_0.55fr_0.75fr_0.9fr_0.7fr]";
...
function StatCards({ counts }: { counts?: BlogCategoryListCounts }) {
  const stats = [
...
function BlogCategoriesPage() {
  const { push } = useToast();
  const [rows, setRows] = useState<BlogCategoryRecord[]>([]);
  const [listCounts, setListCounts] = useState<BlogCategoryListCounts>();
  ...
  const [editingRecord, setEditingRecord] = useState<BlogCategoryRecord | null>(null);
  ...
  const fetchRows = useCallback(async () => {
    setLoading(true);
    try {
      const data = await blogCategoriesService.listBlogCategories({
        page: currentPage,
      ...
      setTotalItems(data.pagination?.totalBlogCategories ?? 0);
      setListCounts(data.counts);
    } catch (error) {
      push({
        type: "error",
        title: "Failed to load blog categories",
        description: getApiErrorMessage(error, "Unable to fetch blog categories."),
      });
      setRows([]);
      setTotalPages(1);
      setTotalItems(0);
  ...
  const handleSave = async (payload: BlogCategoryFormPayload, editingId?: string) => {
    try {
      const body = {
        name: payload.name,
      ...
      if (editingId) {
        await blogCategoriesService.updateBlogCategory(editingId, body);
        push({
          type: "success",
          title: "Category updated",
          description: "Blog category saved successfully.",
        });
      } else {
        await blogCategoriesService.createBlogCategory(body);
        push({
          type: "success",
          title: "Category created",
          description: "New blog category added successfully.",
        });
      }
      setRefreshKey((k) => k + 1);
    } catch (error) {
      push({
        ...
        description: getApiErrorMessage(error, "Could not save blog category."),
      });
    }
  };

  const handleDelete = async (record: BlogCategoryRecord) => {
    const result = await Swal.fire({
      title: "Delete blog category?",
      text: `Remove "${record.name}" and its subcategories? This cannot be undone.`,
      icon: "warning",
    ...
    try {
      await blogCategoriesService.deleteBlogCategory(record._id);
      push({
        type: "success",
      ...
      push({
        type: "error",
        title: "Delete failed",
        description: getApiErrorMessage(error, "Could not delete blog category."),
      });
    }
  };
  ...
  return (
    <div className="px-4 pb-6 pt-4 sm:px-6 lg:px-8">
      <Header title="Blog Categories" showBack={false} onBackClick={() => {}} />
          ...
          <div className="min-w-[1020px]">
            <div className="rounded-[10px] border border-[rgba(34,34,34,0.08)] overflow-hidden bg-white">
              ...
              >
                ...
                <p className="text-[14px] font-[SemiBold] text-[#222]">Subcategories</p>
                ...
                {loading ? (
                  ...
                  rows.map((row, idx) => (
                    ...
                    >
                      ...
                      <p className="text-[12px] font-[Regular] text-[#707070] truncate">{row.slug}</p>
