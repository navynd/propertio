...
import type { PropertyTypeListCounts, PropertyTypeRecord } from "../../../types/api";
...
function StatCards({ counts }: { counts?: PropertyTypeListCounts }) {
  const stats = [
    { label: "Total", value: counts?.totalPropertyTypes ?? 0, accent: "#222" },
    { label: "Active", value: counts?.activePropertyTypes ?? 0, accent: "#00A663" },
    { label: "Inactive", value: counts?.inactivePropertyTypes ?? 0, accent: "#EA3934" },
  ];
...
function PropertyTypePage() {
  const { push } = useToast();
  const [rows, setRows] = useState<PropertyTypeRecord[]>([]);
  const [listCounts, setListCounts] = useState<PropertyTypeListCounts>();
  ...
  const [editingRecord, setEditingRecord] = useState<PropertyTypeRecord | null>(null);
  ...
  const fetchRows = useCallback(async () => {
    setLoading(true);
    try {
      const data = await propertyTypesService.listPropertyTypes({
        page: currentPage,
      ...
      setRows(data.propertyTypes ?? []);
      setTotalPages(Math.max(1, data.pagination?.totalPages ?? 1));
      setTotalItems(data.pagination?.totalPropertyTypes ?? 0);
      setListCounts(data.counts);
    } catch (error) {
      push({
        type: "error",
        title: "Failed to load property types",
        description: getApiErrorMessage(error, "Unable to fetch property types."),
      });
      setRows([]);
      setTotalPages(1);
      setTotalItems(0);
  ...
  const handleSave = async (payload: PropertyTypeFormPayload, editingId?: string) => {
    try {
      if (editingId) {
        await propertyTypesService.updatePropertyType(editingId, payload);
        push({ type: "success", title: "Property type updated", description: "Changes saved." });
      } else {
        await propertyTypesService.createPropertyType(payload);
        push({ type: "success", title: "Property type created", description: "New property type added." });
      }
      setRefreshKey((k) => k + 1);
    } catch (error) {
      push({
        ...
        description: getApiErrorMessage(error, "Could not save property type."),
      });
    }
  };

  const handleDelete = async (record: PropertyTypeRecord) => {
    ...
    try {
      await propertyTypesService.deletePropertyType(record._id);
      push({
        type: "success",
      ...
      push({
        type: "error",
        title: "Delete failed",
        description: getApiErrorMessage(error, "Could not delete property type."),
      });
    }
  };
  ...
  return (
    <div className="px-4 pb-6 pt-4 sm:px-6 lg:px-8">
      <Header title="Property Types" showBack={false} onBackClick={() => {}} />
