...
import type { LanguageRecord } from "../../../types/api";
...
export type LanguageFormPayload = {
  name: string;
  code: string;
  nativeName: string;
  isActive: boolean;
...
type LanguagesModalProps = {
  isOpen: boolean;
  onClose: () => void;
  editingRecord: LanguageRecord | null;
  onSave: (payload: LanguageFormPayload, editingId?: string) => void;
};
...
export default function LanguagesModal({
  ...
  useEffect(() => {
    ...
    if (editingRecord) {
      ...
      setNativeName(editingRecord.nativeName ?? "");
      ...
      <div className="relative bg-white w-full md:max-w-[560px] rounded-t-[15px] md:rounded-[15px] max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-[0px_20px] md:p-[0px_40px]">
          ...
          <div className="flex flex-col gap-[20px] mt-[28px]">
            ...
            <div>
              <label className={labelClass}>Native name</label>
              <input
                type="text"
                value={nativeName}
                onChange={(e) => setNativeName(e.target.value.slice(0, 100))}
                placeholder="e.g. English"
                className={inputClass}
              />
            </div>
        ...
        <div className="shrink-0 p-[20px] md:p-[24px_40px_32px] flex gap-[12px]">
          ...
          >
            {isEdit ? "Save changes" : "Add language"}
          </button>
        </div>
      </div>
    </div>
